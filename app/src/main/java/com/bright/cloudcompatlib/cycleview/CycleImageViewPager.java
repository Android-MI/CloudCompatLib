 ...
 * @Description
 */
public class CycleImageViewPager extends Fragment implements ViewPager.OnPageChangeListener {
    ...
    private CycleImageViewPager.ViewPagerAdapter adapter;
    ...
    final Runnable runnable = new Runnable() {
        public void run() {
            if (CycleImageViewPager.this.getActivity() != null && !CycleImageViewPager.this.getActivity().isFinishing() && CycleImageViewPager.this.isWheel) {
                long now = System.currentTimeMillis();
                if (now - CycleImageViewPager.this.releaseTime > (long) (CycleImageViewPager.this.time - 500)) {
                    CycleImageViewPager.this.handler.sendEmptyMessage(CycleImageViewPager.this.WHEEL);
                } else {
                    CycleImageViewPager.this.handler.sendEmptyMessage(CycleImageViewPager.this.WHEEL_WAIT);
                }
            }

        }
    };
    private CycleImageViewPager.ImageCycleViewListener mImageCycleViewListener;
    private List<Object> infos;

    public CycleImageViewPager() {
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        handler = new CycleViewPageHandler.UnleakHandler(getActivity()) {
            public void handleMessage(Message msg) {
                ...
                if (msg.what == CycleImageViewPager.this.WHEEL && CycleImageViewPager.this.imageViews.size() != 0) {
                    if (!CycleImageViewPager.this.isScrolling) {
                        int max = CycleImageViewPager.this.imageViews.size() + 1;
                        int position = (CycleImageViewPager.this.currentPosition + 1) % CycleImageViewPager.this.imageViews.size();
                        CycleImageViewPager.this.viewPager.setCurrentItem(position, true);
                        if (position == max) {
                            CycleImageViewPager.this.viewPager.setCurrentItem(1, false);
                        }
                    }

                    CycleImageViewPager.this.releaseTime = System.currentTimeMillis();
                    CycleImageViewPager.this.handler.removeCallbacks(CycleImageViewPager.this.runnable);
                    CycleImageViewPager.this.handler.postDelayed(CycleImageViewPager.this.runnable, (long) CycleImageViewPager.this.time);
                } else {
                    if (msg.what == CycleImageViewPager.this.WHEEL_WAIT && CycleImageViewPager.this.imageViews.size() != 0) {
                        CycleImageViewPager.this.handler.removeCallbacks(CycleImageViewPager.this.runnable);
                        CycleImageViewPager.this.handler.postDelayed(CycleImageViewPager.this.runnable, (long) CycleImageViewPager.this.time);
                    }
    ...
    public void setData(List<ImageView> views, List<Object> list, CycleImageViewPager.ImageCycleViewListener listener) {
        this.setData(views, list, listener, 0);
    }

    public void setData(List<ImageView> views, List<Object> list, CycleImageViewPager.ImageCycleViewListener listener, int showPosition) {
        ...
        if (views.size() == 0) {
            ...
            this.adapter = new CycleImageViewPager.ViewPagerAdapter();
            this.setIndicator(0);
    ...
    private class ViewPagerAdapter extends PagerAdapter {
        ...
        public int getCount() {
            return CycleImageViewPager.this.imageViews.size();
        }
        ...
        public View instantiateItem(ViewGroup container, int position) {
            ImageView v = CycleImageViewPager.this.imageViews.get(position);
            if (CycleImageViewPager.this.mImageCycleViewListener != null) {
                v.setOnClickListener(new View.OnClickListener() {
                    public void onClick(View v) {
                        CycleImageViewPager.this.mImageCycleViewListener.onImageClick(CycleImageViewPager.this.infos.get(CycleImageViewPager.this.currentPosition - 1), CycleImageViewPager.this.currentPosition, v);
                    }
                });
            }
